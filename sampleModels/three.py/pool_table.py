...
def pool_table(L_table=2.3368, W_table=None, H_table=0.74295,
    ...
    headSpotMaterial = MeshBasicMaterial(name="headSpotMaterial", color=0x777777, shading=FlatShading)
    surfaceMaterial = MeshBasicMaterial(name="surfaceMaterial", color=0x00aa00, shininess=5, shading=FlatShading)
    cushionMaterial = MeshBasicMaterial(name="cushionMaterial", color=0x028844, shininess=5, shading=FlatShading)
    railMaterial = MeshBasicMaterial(name="railMaterial", color=0xdda400, shininess=10, shading=FlatShading)
...
with open(output, 'w') as f:
    f.write(json.dumps(pool_hall().export(), indent=2, sort_keys=True))
